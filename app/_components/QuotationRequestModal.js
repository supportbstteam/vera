...
const QuotationRequestModal = ({ handleModalType }) => {
  ...
  const fetch_data = async () => {
    try {
      ...
      let selected_category =
        localStorage.getItem(process.env.APP_PREFIX + "selected_category") ??
        "";
      selected_category = JSON.parse(selected_category);
      const search_text =
        localStorage.getItem(process.env.APP_PREFIX + "search_text") ?? "";
  ...
  const handleFileChange = async (e) => {
    ...
    if (
      file_size > maxFileSizeInBytes ||
      allowedExtensionsArr.indexOf(file_type) < 0
    ) {
      let error_txt =
        "Please Upload only " +
        allowedExtensions +
      ...
      try {
        ...
        if (res && res.status === 200) {
          ...
          localStorage.removeItem(
            process.env.APP_PREFIX + "selected_category_time"
          );
  ...
  return (
    <section className="max-w-2xl m-auto p-6 max-h-[85vh] overflow-auto">
      <div>
      ...
      <form
        ...
        ref={formRef}
      >
        <div className="grid grid-cols-1 md:grid-cols-[2fr_3fr] justify-start  items-center gap-4 md:gap-6 mt-4 md:mt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-6 my-6">
          <div className={`grid grid-cols-1 mb-3`}>
            <Input
              label="First Name"
              autoComplete="off"
        ...
        <div className="md:flex justify-between items-end mt-4 ">
          ...
          <SbButton
            data={{
              ...
              class:
                "mt-4 md:mt-0 inline-flex items-center justify-center rounded-md font-medium transition-colors px-4 py-2 text-base bg-primary text-white hover:text-gray-800 hover:bg-primary/80 hover:text-white",
