        ...
        <div key={item.id}>
          <div className="flex items-center justify-between gap-4 py-4">
            ...
            <div className="w-6/12 flex items-center gap-4 ">
              <Image src={item.image} alt={item.title} width={80} height={50} className="rounded-md" />
              <div className="">
                <h3 className="font-medium text-base">{item.title}</h3>
                <p className="text-sm text-gray-500">Posted on : {item.postedOn}</p>
              </div>
            </div>
            ...
            <div className="w-6/12 flex items-center justify-between gap-4 ">
              <span className="text-sm bg-purple-100 text-purple-600 px-3 py-2 rounded-full">
                {String(item.vendorInterest).padStart(2, "0")} Vendor Show Interest
              </span>

              <span className={`text-sm px-3 py-2 rounded-full flex items-center gap-1 ${statusColors[item.status]}`}>
